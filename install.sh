...
# DNA patch: snapshot .env.local before the rewrite below, so keys this installer
# does not know about survive it.
DNA_ENV_SNAPSHOT=""
if [[ -f "$ENV_FILE" ]]; then
  DNA_ENV_SNAPSHOT="$(mktemp)"
  cp "$ENV_FILE" "$DNA_ENV_SNAPSHOT"
fi
...
# DNA patch: put back what the rewrite dropped. The heredoc emits a fixed set of
# keys and preserves eleven more by name; anything else written beforehand -
# branding, SMTP, the Stripe publishable key - was silently lost. That matters
# more than it looks: the build a few steps down inlines every NEXT_PUBLIC_*
# value into the client bundle, so a key missing here is missing from the
# shipped JavaScript, and the portal comes up unbranded and unable to send mail.
if [[ -n "$DNA_ENV_SNAPSHOT" ]]; then
  while IFS= read -r dna_line; do
    [[ "$dna_line" =~ ^[A-Za-z_][A-Za-z0-9_]*= ]] || continue
    dna_key="${dna_line%%=*}"
    grep -q "^${dna_key}=" "$ENV_FILE" || printf '%s\n' "$dna_line" >> "$ENV_FILE"
  done < "$DNA_ENV_SNAPSHOT"
  rm -f "$DNA_ENV_SNAPSHOT"
fi
